...
public class VolleyConnection {
    ...
    public String getRouteURL(List<LatLng> waypoints) {
        ...
        str_waypoint += waypoints.get(waypoints.size() - 2).latitude + "," + waypoints.get(waypoints.size() - 2).longitude;

        String trafficMode = "mode=walking";

        // Building the parameters to the web service
        String parameters = str_origin + "&" + str_dest + "&" + trafficMode + "&" + str_waypoint;
        //json?origin=Adelaide,SA&destination=Adelaide,SA&waypoints=optimize:true|Barossa+Valley,SA|Clare,SA|Connawarra,SA|McLaren+Vale,SA&key=YOUR_API_KEY
